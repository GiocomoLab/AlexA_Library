...
session_name = {};
sn = dir(fullfile(data_dir,'AA*_gain_*.mat'));
for iS = 1:numel(sn)
...
for session_num = 1%10:numel(session_name)    
    % load data
    ...
    image_save_dir = strcat('F:\images\',...
        session_name{session_num},'\pretty_rasters_speed\');
    ...
    %% calculate trial_speed
    speed = [0; diff(posx)];
    speed(speed<-5)=nan;
    speed = 0.5 * (fillmissing(speed, 'previous') + fillmissing(speed, 'next'));
    posWindow= [140 160];
...
for ii=1:max(trial)
    ...
    h = figure('Position',[109   487   742   259]); hold on;
