...
x1=pval;
for iT=1:15
delta_c = CORR_VEC(:,(iT-1)*nChunks+iJ)-CORR_VEC(:,iT*nChunks+iJ);
for ii=1:size(CORR_VEC,2);
    mdl = fitlm(SHIFT_VEC(:,ii),delta_c);
...
subplot(11,1,[1:5])
hold on
boundedline(x_vec(take_idx),tmp_corr,std(CV(idx,take_idx))/sqrt(nnz(idx)),'cmap',CT(iS+2,:),'alpha')

%plot(x_vec,tmp_corr,'Color',CT(iS+2,:))
subplot(11,1,[6:10])
hold on
boundedline(x_vec(take_idx),tmp_shift,std(SV(idx,take_idx))/sqrt(nnz(idx)),'cmap',CT(iS+2,:),'alpha')
