...
subplot(1,2,1)
boundedline(data.X,nanmean(data.Y),nanstd(data.Y)/sqrt(size(data.Y,1)),'alpha','cmap',cm(ii,:))
hold on
axis tight
ylim([0 0.8])
leg{end+1}=gains{ii};
...
boundedline(data.X,nanmean(data.S*2),nanstd(data.S*2)/sqrt(size(data.Y,1)),'alpha','cmap',cm(ii,:))
    axis tight
    ylim([-2 6])
    catch ME
    end
