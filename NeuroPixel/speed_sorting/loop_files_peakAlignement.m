...
savepath=fullfile(OAK,'attialex','peak_shift22cmFilt_16prec');
...
parfor iF=1:numel(filenames)
    try
        ...
        for iRep = 1:min(2,numel(triggers{iF}))
            ...
            data_peak = findPeakAlignement(data,ops_temp);
            data_peak.trials = ops_temp.trials;
            data_out{iRep} = data_peak;
            mf = matfile(fullfile(savepath,sprintf('%s_%d',sn,iRep)),'Writable',true);
            %fn = fieldnames(data_peak);
            
            mf.data_peak = data_peak;
