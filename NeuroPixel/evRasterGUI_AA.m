...
function f = evRasterGUI_AA(st, clu, eventTimes)
...
pars.nTop = 2; % num subplots in the top two rows
pars.nBottom = 7;
...
pars.tickSize = ceil(numel(eventTimes)/300);
...
evData = createEvTimesOrders(eventTimes);
...
function evData = createEvTimesOrders(eventTimes)
fn=fieldnames(eventTimes);
for n=1:length(fn)
% top row, move offset before trial start
evData(n).times = eventTimes.(fn{n});
evData(n).trOrders = [1:numel(eventTimes.(fn{n}))]';
evData(n).windows = [-0.3 0.7];
evData(n).groups = ones(size(evData(n).trOrders));
evData(n).gIDs = 1;
evData(n).colors = [0 .5 0];
evData(n).alignName = fn{n};
