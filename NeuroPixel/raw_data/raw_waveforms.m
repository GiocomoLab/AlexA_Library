...
ksDirs = dir(fullfile('/oak/stanford/groups/giocomo/export/data/Projects/ContrastExperiment_neuropixels/H3/neuropixels_data/**/**/*.ap.bin'));
%dataset to load
...
im_root = fullfile('/oak/stanford/groups/giocomo','attialex','images');
%im_root = 'F:\images';

%%
for iDir = 4%1:numel(ksDirs)
...
%     bar(b,CGR,'EdgeColor','none')
    ...
    pbaspect([1.5,4,1])
    %saveas(gcf,fullfile(im_save_dir,sprintf('%d.svg',cluid)))
    
    %print(fullfile(im_save_dir,sprintf('%d.png',cluid)),'-dpng','-r600')
    %print(fullfile(im_save_dir,sprintf('%d.svg',cluid)),'-dsvg')
