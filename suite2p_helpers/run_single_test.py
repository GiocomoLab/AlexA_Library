...
def run_for_single_file(animal_path,file):
    ...
    if not os.path.isdir(ops['save_path0']):
        ...
        stop = time.time()
        timelog = open(os.path.join(os.environ['L_SCRATCH'],'suite2p_times.txt'),'a')
...
def run_for_animalDir(animal_path):
    mkdir_p(os.path.join(os.environ['SCRATCH'],'register_jobs'))
    mkdir_p(os.path.join(os.environ['SCRATCH'],'register_jobs','bin_files'))
